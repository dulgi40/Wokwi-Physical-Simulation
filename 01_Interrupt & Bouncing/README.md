...
### 💡 4. 考察 (Insight)
1.  **ハードウェア的な代替案:** コストやスペースに余裕がある場合、
2.  **RCフィルタ(抵抗とコンデンサ)**
3.  やシュミットトリガ回路を構成することで、MCUに信号が届く前に物理的にノイズを
4.  **除去**
5.  できることを学んだ。これはCPUリソースを節約できる最も確実な方法である。
6.  **`delay()` 使用の注意点:** 割込みシステムにおいて `delay()` のようなブロッキング関数は、システムの応答性を低下させ、割込みの即時性を
7.  **妨げる**
8.  可能性がある。そのため、`millis()` ベースの非同期処理が必須であることを体感した。
