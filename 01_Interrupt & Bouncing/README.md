...
* **ロジック:** ISRの内部に「検問所」のようなロジックを設置し、前回の割込み発生時点から一定時間(100ms)が経過していない場合は、追加の信号をノイズとして無視するように設計した。
...
2.  **`delay()` 使用の注意点:** 割込みシステムにおいて `delay()` のようなブロッキング関数は、システムの応答性を低下させ、割込みの即時性を**妨げる(さまたげる)**可能性がある。そのため、
